...
#pragma warning disable CA1308 // Yes, we want to use lower case characters
    ...
    */
    ...
    public partial class FormMain : Form
    {
        ...
        */
        // =================================================================================
        public bool SetupCamerasPageVisible { get; set; } = false;
        ...
        public void My_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                JoggingBusy = true;
                Cnc.RawWrite(Movestr + "Y" + Setting.General_MachineSizeY.ToString(CultureInfo.InvariantCulture) + "\"}");
            }
        ...
        private void General_pictureBox_MouseMove(int MouseX, int MouseY)
        {
            ...
            {
                DisplayText("X: " + MouseX.ToString(CultureInfo.InvariantCulture) + ", Y: " + MouseY.ToString(CultureInfo.InvariantCulture));
            }
        }
        ...
        private void tabPageSetupCameras_Begin()
        {
            ...
            {
                ShowMessageBox(
                   "Unknown fiducial type " + Setting.Placement_FiducialsType.ToString(CultureInfo.InvariantCulture)
                   + ", possibly corrupted settings",
                   "Corrupted settings",
                   MessageBoxButtons.OK);
                RoundFiducial_radioButton.Checked = true;
            }
        ...
        */
        ...
        private bool LoopTinyGParameters()
        {
            ...
            foreach (var parameter in TinyGBoard.GetType().GetProperties())
            {
                ...
                // to separate TinyG and other boards settings, a prefix XXX_ is used. Remove that:
        ...
        private void Zvm_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == '\r')
            {
                ...
                int peek = Convert.ToInt32(zvm_maskedTextBox.Text, CultureInfo.InvariantCulture);
        ...
        private void Avm_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == '\r')
            {
                ...
                int peek = Convert.ToInt32(avm_maskedTextBox.Text, CultureInfo.InvariantCulture);
            ...
            if (LoadCadData_m())
            {
                ...
                if (File.Exists(JobFileName))
                {
                    if (!LoadJobData_m(JobFileName))
                    {
                        ...
                        FindFiducials_m(out int dummy);     // don't care of the result, just trying to find fids
                    }
                }
                else
                {
                    ...
                    FillJobData_GridView();
                    if (FindFiducials_m(out int dummy))     // don't care of the result, just trying to find fids
                    {
        ...
        private bool ParseKiCadData_m(String[] AllLines)
        {
            ...
            while (!(AllLines[i]).StartsWith("## End", StringComparison.Ordinal))
            {
        ...
        private void DebugNozzle()
        {
            ...
            Xpx = X * UpCamera.GetMeasurementZoom();
            Ypx = Y * UpCamera.GetMeasurementZoom();
            DisplayText(res.ToString(CultureInfo.InvariantCulture) + " candidates, smallest: ");
            ...
            DisplayText("X: " + Xpx.ToString(CultureInfo.InvariantCulture)
                + " pixels, Y: " + Ypx.ToString(CultureInfo.InvariantCulture) + " pixels");
            X = X * Setting.UpCam_XmmPerPixel;
            Y = -Y * Setting.UpCam_YmmPerPixel;
            DisplayText("X: " + X.ToString("0.000", CultureInfo.InvariantCulture));
            DisplayText("Y: " + Y.ToString("0.000", CultureInfo.InvariantCulture));
        ...
        private void SetEditTargets()
        {
            ...
            switch (Display_dataGridView.Rows[row].Cells[FunctCol].Value.ToString())
            {
                ...
                case "Meas. zoom":
                    ...
                    {
                        Parameter_double_textBox.Text = par_d.ToString("0.0", CultureInfo.InvariantCulture);
                    }
                ...
                case "Gaussian blur":
                    ...
                    {
                        Parameter_double_textBox.Text = par_d.ToString("0.00", CultureInfo.InvariantCulture);
                    }
                    Parameter_double_textBox.Enabled = true;
        ...
        private void Parameter_Int_numericUpDown_ValueChanged(object sender, EventArgs e)
        {
            ...
            int row = Display_dataGridView.CurrentCell.RowIndex;
            int IntCol = (int)Display_dataGridViewColumns.Int;
            Display_dataGridView.Rows[row].Cells[IntCol].Value = Parameter_Int_numericUpDown.Value.ToString(CultureInfo.InvariantCulture);
            UpdateDisplayFunctions();
        }
        ...
        private void R_numericUpDown_ValueChanged(object sender, EventArgs e)
        {
            ...
            int row = Display_dataGridView.CurrentCell.RowIndex;
            int R_col = (int)Display_dataGridViewColumns.R;
            Display_dataGridView.Rows[row].Cells[R_col].Value = R_numericUpDown.Value.ToString(CultureInfo.InvariantCulture);
            SetColorBoxColor(row);
            UpdateDisplayFunctions();
        }
        ...
        private void G_numericUpDown_ValueChanged(object sender, EventArgs e)
        {
            ...
            int row = Display_dataGridView.CurrentCell.RowIndex;
            int G_col = (int)Display_dataGridViewColumns.G;
            Display_dataGridView.Rows[row].Cells[G_col].Value = G_numericUpDown.Value.ToString(CultureInfo.InvariantCulture);
            SetColorBoxColor(row);
            UpdateDisplayFunctions();
        }
        ...
        private void B_numericUpDown_ValueChanged(object sender, EventArgs e)
        {
            ...
            int row = Display_dataGridView.CurrentCell.RowIndex;
            int B_col = (int)Display_dataGridViewColumns.B;
            Display_dataGridView.Rows[row].Cells[B_col].Value = B_numericUpDown.Value.ToString(CultureInfo.InvariantCulture);
            SetColorBoxColor(row);
            UpdateDisplayFunctions();
        }
        ...
        private bool m_GetNozzleStartCoordinates(DataGridView grid, int nozzle, out double X, out double Y, out double Z)
        {
            ...
            {
                DisplayText("Bad data, Start X, " + op + " nozzle #" + nozzle.ToString(CultureInfo.InvariantCulture));
                return false;
            }
            if (!NozzleDataCheck(grid, nozzle, Nozzledata_StartYColumn, out Y))
            {
                DisplayText("Bad data, Start Y, " + op + " nozzle #" + nozzle.ToString(CultureInfo.InvariantCulture));
                return false;
            }
            if (!NozzleDataCheck(grid, nozzle, Nozzledata_StartZColumn, out Z))
            {
                DisplayText("Bad data, Start Z, " + op + " nozzle #" + nozzle.ToString(CultureInfo.InvariantCulture));
                return false;
            }
            return true;
        }
        ...
        private void GetCoordinates_button_Click(DataGridView grid)
        {
            ...
            int row = grid.CurrentCell.RowIndex;
            int col = grid.CurrentCell.ColumnIndex;
            DisplayText("Current Cell: " + row.ToString(CultureInfo.InvariantCulture) + ", " + col.ToString(CultureInfo.InvariantCulture));
